...
public class PlayerController : BaseEntityComponent, IPlayerMovable, IMoveInput, IInteractInput, IRidingInput,IJumper,IRunner,IModel
{
    ...
    public void OnMoveInput(Vector2 inputDir)
    {
        ...
        if(!State.HasState(PlayerState.Move)) State.SetState(PlayerState.Move);
    ...
    {
        if (State.CurrentState == PlayerState.Die || State.HasState(PlayerState.Riding)||interact.CurrentTarget == null) return;
